  ...
  <div>
      ...
      <div class="plan_box">
        ...
        <div class="meter_nums clearfix">
          ...
          <div class="meter_box meter_box3">
            <p>本次读书</p>
            <el-input v-model="param.ReadNum" placeholder="按[enter]键确定" ref="ReadNumInput" @keyup.enter.native="getWaterPredict(1)" onkeyup="value=value.replace(/\D/g,'')"></el-input>
          </div>
    ...
    <div class="meter_Small" v-else>
      ...
      <div class="meter_nums clearfix">
        ...
        <div class="meter_box meter_box3">
          <p>本次读书</p>
          <el-input v-model="param.ReadNum" ref="ReadNumInput" placeholder="按[enter]键确定" @keyup.enter.native="getWaterPredict(1)"></el-input>
        </div>
        <el-input class="meter_remark" placeholder="请输入内容" v-model="param.Remark">
          <template slot="prepend">备注：</template>
        </el-input>
      ...
      getCheck(param){//抄表
        getReading(param).then(res => {
          if (res.code ==0 ) {
             ...
             * 待用户模块数据提供之后完善
             * */
            if(this.checks.indexOf('自动载入下一户')=='-1'){//是否自动载入下一户
              alert('待完善')
              //this.$parent.$refs.multipleTable.setCurrentRow(this.$parent.$refs.multipleTable.data[2]);
          ...
          if(curNum !=''){
            ...
            if(this.checks.indexOf('字轮是否翻页')=='-1'){//当未勾选字轮翻页,判断本次读书必须大于上次读书
              ...
              if(lastRead>0&&lastRead>curNum){
                curWater = parseInt(totalNum) - lastRead + parseInt(curNum) //本次水量=总数-上次读书+本次读书
