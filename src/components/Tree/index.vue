  ...
  export default {
    ...
    methods: {
      ...
      renderContent(h, {node, data, store}) {
        return ( <span slot-scope = '{ node, data }' style = 'margin-left:12px' id= {data.Id}> {node.label} < /span>)
      },
      /**
       * 当前选中需编辑或者新增信息或删除
       * */
      ...
      getParentId(keyword, data) {
        ...
        let pidArr = []
        let flagArr = []
        flagArr.push({Id: data.Id, label: data.label, flag: data.label.indexOf(keyword) != -1})//当前节点ID，及label名
        pidArr.push(flagArr)
      ...
      getChildId(keyword, data, pidArr) {//如果节点有下级节点，则往下遍历获取每一层ID 是否与当前选中ID 相等
        if (data.children.length > 0) {
          data.children.map(item => {
            let flagArr = []
            flagArr.push({Id: item.Id, label: item.label, flag: item.label.indexOf(keyword) != -1})
        ...
        if (count === this.idsLength + 1) {//二级节点+1 其中1为最顶层父节点,获取最后一层遍历之后数据集合
      ...
      treeClickChecked(theTree, manualNode) {
        ...
        manualNode.forEach((item) => {//过滤选中节点与未选中节点集合
          ...
          if (JSON.stringify(item).indexOf(JSON.stringify(true)) === -1) {//包含二级节点的未选中节点
        ...
        if (this.macthArray.length > 0)
          this.macthArray.forEach((items) => {//过来当前模糊查询所匹配节点
            items.forEach((i) => {
              Nodes.forEach((key) => {
        ...
        if (this.noneArry.length > 0)
          this.noneArry.forEach((items) => {//过来当前模糊查询所匹配节点
            items.forEach((i) => {
